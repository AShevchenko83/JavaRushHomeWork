...
public class Solution {
    ...
    public static String convertToDecimalSystem(String s) {
        ...
        if(s.charAt(0)=='0'){
            if(s.charAt(1)=='x'){
                return new Integer(Integer.parseInt(s.substring(2), 16)).toString();
            }
            else if(s.charAt(1)=='b'){
                return new Integer(Integer.parseInt(s.substring(2), 2)).toString();
            }
            else return new Integer(Integer.parseInt(s.substring(1), 8)).toString();
        }
        else return new Integer(s).toString();
    }
}
